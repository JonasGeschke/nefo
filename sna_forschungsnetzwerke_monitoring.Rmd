---
title: "Soziale Netzwerkanalyse: NeFo-Forschungsatlas und Monitoring"
...
setwd("Z:/Benutzer/Jonas.Geschke/PPPs/Monitoring_2018_TERENO/Netzwerkanalyse") # Buero
```
...
## Load raw data
Raw_forschungsnetzwerk <- read.csv2("forschungsnetzwerk.csv", sep=",", dec=".", header=T, encoding="UTF-8")

## Prepare dataset "FN"
Gdata_FN <- Raw_forschungsnetzwerk
names(Gdata_FN) <- c("inst", "abbr", "bayklif", "BeGenDiv", "BBIB", "BioFrankfurt", "BION", "iDiv", "Ecornet", "FVB", "GBIF-D", "GFBIO", "HIFMB", "KDM", "LVB")
Gdata_FN <- Gdata_FN[,-10] #Delete FVB
rownames(Gdata_FN) <- Gdata_FN[,2] #Inst oder abbr as rownames
Gdata_FN <- Gdata_FN[,-c(1,2)] #Delete inst and abbr
Gdata_FN <- Gdata_FN[,c(12,9,5,6,1,3,11,4,7,8,2,10)] #Order by number of members

Gdata_FN$bayklif <- as.numeric(Gdata_FN$bayklif)
...
```{r Laden der Rohdaten und Vorbereitung der Daten, eval=FALSE}
## Load raw data
Raw_themen <- read.csv2("querschnitts-ipbes-themen.csv", sep=",", dec=".", header=T, encoding="UTF-8")

## Prepare dataset "FN"
Gdata_BM <- Raw_themen
Gdata_BM <- Gdata_BM[,c(1,2,7)]
names(Gdata_BM) <- c("inst", "abbr", "monitoring")

Gdata_BM$monitoring <- as.numeric(Gdata_BM$monitoring)
df <- as.data.frame(matrix(data = NA, nrow = 12, ncol = 3), stringsAsFactors = FALSE)
names(df) <- c("inst", "abbr", "monitoring")
Gdata_BM <- rbind(Gdata_BM, df)
rm(df)
```
...
V(fn2)$monitoring <- Gdata_BM$monitoring
...
farben10 <- c("greenyellow", "green", "forestgreen", "darkseagreen", "black", "cornflowerblue", "lightslateblue", "navy", "red", "deeppink2", "chocolate4", "black")
coul10 <- c()
rowno <- c(1:nrow(links_chord))
for (r in rowno){
...
farben_fn2[(length(degree(fn2))-(ncol(Gdata_FN)-1)):length(degree(fn2))] <- "dark grey"

palette_fn2_cut <- as.numeric(degree(fn2_cut)+1) # Farbpalette entsprechend vertex.size
farben_fn2_cut <- coul6[palette_fn2_cut]
farben_fn2_cut[(length(degree(fn2_cut))-(ncol(Gdata_FN)-1)):length(degree(fn2_cut))] <- "dark grey"
...
V(fn2)$monitoringcolor[V(fn2)$monitoring == 1] <- "forestgreen"
V(fn2_cut)$monitoringcolor[V(fn2_cut)$monitoring == 1] <- "forestgreen"
...
plot(fn2, vertex.size = 3, vertex.color = farben_fn2,
     ...
     vertex.frame.color = V(fn2)$monitoringcolor, vertex.label.color = "black",
     edge.arrow.size = 0,
     layout = lyout_fn2)
