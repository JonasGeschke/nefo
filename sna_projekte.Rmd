...
Raw_projects_gesamt <- read.csv2("Daten/Suchliste_gesamt_form.csv", sep=";", dec=",", header=T, stringsAsFactors=F) # Abgerufen am 9.1.19
...
Gdata$Förderzeitraum <- NA
Gdata$calc <- NA
Gdata <- Gdata[,c(1:2,17,18,3:16)]

#Gdata$Laufzeit.von <- strptime(Gdata$Laufzeit.von, format = "%d.%m.%Y")
#Gdata$Laufzeit.bis <- strptime(Gdata$Laufzeit.bis, format = "%d.%m.%Y")
...
for(n in nrow){
  p <- 1
  Gdata[n,3] <- paste(as.numeric(format(as.Date(Gdata[n,5], "%d.%m.%Y"), "%Y")),
                      #strrep((as.numeric(format(as.Date(Gdata[n,5], "%d.%m.%Y"), "%Y")) + p), years[n]), sep = "/")
                                (repeat{
                                  c(as.numeric(format(as.Date(Gdata[n,5], "%d.%m.%Y"), "%Y")) + p)
                                  p <- p + 1
                                  if(p == years[n]){break}}), sep = "/")
}
...
proj <- data.frame(id = Gdata$proj_id,
                   ...
                   budget = Gdata$Fördersumme.in.EUR,
                   stringsAsFactors = FALSE)
...
V(ntw)$plotname[grep("Museum für Naturkunde Leibniz-Institut", V(ntw)$name)] <- "MfN"
V(ntw)$plotname[grep("Helmholtz-Zentrum für Umweltforschung GmbH - UFZ", V(ntw)$name)] <- "UFZ"
V(ntw)$plotname[grep("Senckenberg Gesellschaft für Naturforschung", V(ntw)$name)] <- "SGN"
...
plot(ntw, vertex.size = 3, vertex.color = V(ntw)$vertex.col,
     ...
     layout = lyout,
     main = "biodiv + biologische Vielfalt")
