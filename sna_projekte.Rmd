...
Suche <- paste("http://gepris.dfg.de/gepris/OCTOPUS?context=project&findButton=Finden&hitsPerPage=10000&keywords_criterion=", keywords, "&language=de&nurProjekteMitAB=false&task=doSearchSimple", sep = "") # Förderjahr bearbeiten &gefoerdertIn=2018
...
for(a in letters){
  ...
  INDEX <- rbind(INDEX,
                 Rcrawler(Suche, no_cores = 4, no_conn = 4,
...
#MyDATA <- LoadHTMLFiles("gepris.dfg.de-111357", type = "list")
DATA <- ContentScraper(Url = c(as.vector(INDEX$Url)),
...
column_names <- gsub("[\r\n\t]", "", column_names)
content <- unlist(lapply(DATA, `[[`, 4))
content <- gsub("[\r\n\t]", "", content)

projno <- c(1:length(DATA))
contentno <- vector()
for(p in projno){
...
names(gepris) <- c("titel", "descr", "name", "content", unique(column_names))
...
for(n in projno){
  ...
  for(c in check){
    ...
    if(name[c] == names(gepris[a])){gepris[n,a] <- value[c]}
    }
  }
}
...
gepris$titel <- as.character(gepris$titel)
gepris$descr <- as.character(gepris$descr)
