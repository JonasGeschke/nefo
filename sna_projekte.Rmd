...
Gdata <- Gdata[-which(Gdata$Klartext.Leistungsplansystematik %in% c("Monitoring")),]#, 
                                                                    "Mensch-Technik-Interaktion für den demografischen Wandel (abgeschlossen)")),]
...
V(ntw)$vertex.size[V(ntw)$type == "Projekt"] <- 1.5
...
plot(ntw, vertex.size = V(ntw)$vertex.size, vertex.color = V(ntw)$vertex.col,
     ...
     layout = lyout,
     main = "Projektbasierte Vernetzung im Biodiversitätsmonitoring 2019")
legend(-1.6,-0.5, title = "Institution: Degree", legend = c("1", "2-5", "6-10", "11-15", ">15"), fill = coul)
legend(1,-0.5, title = "Projekt: Förderer", legend = c("BMBF", "BMEL", "BMU", "BMWi"), fill = ressort)
