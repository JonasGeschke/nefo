...
gepris[,1] <- "DFG" # Ressort
gepris[,6] <- Raw_gepris[,1] # Thema
gepris[,7] <- Raw_gepris[,13] # Verbundprojekt
gepris[,13] <- Raw_gepris[,6] # Förderprofil
gepris[,14] <- Raw_gepris[,3] # Leistungsplansystematik
...
gepris[,3] <- laufzeit[,1] # von
gepris[,4] <- laufzeit[,2] # bis
rm(laufzeit,r)
...
gepris[,9] <- antragsteller[,1]
rm(antragsteller)

#gepris <- gepris[-which(gepris[,9] == "NA"),]
test <- gepris[which(grepl("[0-9]:", substr(gepris[,6], 1, 10))),]
...
Gdata <- subset_förderzeitraum(years = c(2007, 2008, 2009, 2010, 2011, 
