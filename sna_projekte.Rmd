...
files <- c(1:length(list.files(path = "U:/Netzwerkanalyse_Projekte/R/gepris.dfg.de-111357")))

DATA <- list()
for(a in files){
  DATA[[a]] <- ContentScraper(HTmlText = paste("U:/Netzwerkanalyse_Projekte/R/gepris.dfg.de-111357/", a," .html", sep=""), 
                       # MyDATA[[1]], # Url = c(as.vector(INDEX$Url)),
...
```{r Laden der Rohdaten und Vorbereitung der Daten: Ministerien, eval=FALSE}
library(xlsx)
## Load raw data
...
gepris$Ressort <- "DFG"
gepris$Thema <- Raw_gepris$titel
gepris$Verbundprojekt <- Raw_gepris$`Teilprojekt zu`

laufzeit <- data.frame("von"=as.character(NA), "bis"=as.character(NA), "förderung"=as.character(Raw_gepris$Förderung),
                       stringsAsFactors=FALSE)
str(laufzeit)
laufzeit[which("in" %in% laufzeit$förderung)]

laufzeit$von <- readr::parse_number(laufzeit$förderung)
for(r in zeilen){
  if(grepl("in", laufzeit[r,3])){laufzeit[r,2] <- laufzeit[r,1]}
  if(grepl("seit", laufzeit[r,3])){laufzeit[r,2] <- format(Sys.Date(), "%Y")}
}

tidyr::extract_numeric(laufzeit$förderung)

laufzeit$bis <- as.numeric(laufzeit$bis)
