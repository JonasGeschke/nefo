...
keywords <- biodiv*+OR+%26quot%3Bbiologische+vielfalt%26quot%3B+OR+%26quot%3Bbiologischen+vielfalt%26quot%3B+OR+%26quot%3Bbiological+diversity%26quot%3B+OR+%26quot%3Bmonitoring%26quot%3B # * = und Anhang | +OR+ = OR | + = Leerzeichen | %26quot%3B = Anführungsstriche
Suche <- paste("http://gepris.dfg.de/gepris/OCTOPUS?context=project&findButton=Finden&hitsPerPage=10000&keywords_criterion=", keywords, "&gefoerdertIn=2018&language=de&nurProjekteMitAB=false&task=doSearchSimple", sep = "") # Förderjahr bearbeiten
...
MyDATA <- LoadHTMLFiles("gepris.dfg.de-281459", type = "vector")
gepris <- ContentScraper(Url = c(as.vector(INDEX$Url))[1:5], 
                         XpathPatterns = c("/html/body/div[1]/div/div[3]/div[1]/div/div/div[2]/div[1]/div[1]/h3",
                                           "/html/body/div[1]/div/div[3]/div[1]/div/div/div[2]/div[1]/div[1]/div[4]/span[2]",
                                           "/html/body/div[1]/div/div[3]/div[1]/div/div/div[2]/div[1]/div[1]/div[1]/span[2]",
                                           "/html/body/div[1]/div/div[3]/div[1]/div/div/div[2]/div[1]/div[1]/div[2]/span[2]"),
#                         CssPatterns = c(".details > h3:nth-child(1)",
 #                                        ".details > div:nth-child(3) > span:nth-child(2)"), zeitraum
                         PatternsName = c("Feld 1",
                                          "Feld 2",
                                          "Feld 3",
                                          "Feld 4"),
                         ManyPerPattern = TRUE)
...
Rcrawler("http://www.biodiversity.de", no_cores = 4, no_conn = 4, 
         ignoreUrlParams = c("image", "icon", "logo", "png", "jpg", "jpeg", "user", "file"), 
         NetworkData = TRUE, NetwExtLinks =TRUE,  statslinks = TRUE)
library(igraph)
network <- graph.data.frame(NetwEdges, directed=T)
lyout <- layout.fruchterman.reingold(network)
E(network)$edge.col[E(network)$Type == 1] <- "green"
E(network)$edge.col[E(network)$Type == 2] <- "light blue"
plot(network, vertex.size = 1, vertex.color = "forest green", 
     vertex.label = NA, vertex.label.family = "Calibri", 
     vertex.label.cex = NA, vertex.frame.color = NA, vertex.label.color = NA,
     edge.arrow.size = 0.1, edge.color = E(network)$edge.col,
     layout = lyout)
```
...
raus <- leistungsplan$Klartext.Leistungsplansystematik
```
...
Gdata <- subset_förderzeitraum(years = c(2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018), 
                               dataset = biodivbiolvielfalt)

Gdata <- subset_förderzeitraum(years = c(2019), dataset = monitoring)
Gdata <- Gdata[-which(Gdata$Klartext.Leistungsplansystematik %in% NAs),] #c("Monitoring")),]#, 
