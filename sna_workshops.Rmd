...
rm(rows1, rows2)
xlsx::write.xlsx(workshops_sna, file = "SNA_Statistik_Workshops_alle.xls") ### EDIT
...
plotK <- data.frame(n = c(length(which(V(ws2)$kategorie == "Wissenschaft")),
                          ...
                          length(which(V(ws2)$kategorie == "Medien")),
                          length(which(V(ws2)$kategorie == "keine Angabe"))))
plot <- barplot(plotK[-6,], col = c(rgb(92,139,255,max=255), 
                            rgb(219,174,0,max=255), 
                            rgb(11,33,94,max=255), 
                            rgb(0,155,68,max=255), 
                            rgb(180,33,0,max=255)),
        ylim = c(0,120), yaxt='n',
        names.arg=c("Wissenschaft", "Behörde", "Wirtschaft", "Verbände", "Medien"))
text(x = plot, y = plotK[-6,], label = plotK[-6,], pos = 3, cex = 0.8, col = "black")
...
zeitlich <- read.xlsx(file = "Ergebnisse/workshops_zeitlich.xlsx", sheetIndex = 1, encoding="UTF-8")
...
# Dichte pro Jahr
plot(zeitlich$Dichte ~ zeitlich$Jahr, col = "blue",
     xlab = "Jahr",
...
legend("bottomright", legend = c("Netzwerk-Dichte", "GLM der Anzahl Institutionen / 100"), 
       fill = c("blue", "dark grey"),
       bty = "n", cex = 1)
...
>>>>>>> 6ea14dcc85aefd8daf33fb99a922baf86f697aeb
