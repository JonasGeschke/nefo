...
clp_model <- function(graph, times = 100){
  if("igraph" %in% rownames(installed.packages()) == FALSE){
    install.packages("igraph")}
  library(igraph)
  if("TOSTER" %in% rownames(installed.packages()) == FALSE){
...
#    n <- n+1
#    if(n == 100){
#    if(as.data.frame(table(lengths(output)))[which(grepl(median(lengths(output)), as.data.frame(table(lengths(output)))[,1])),2] / sum(table(lengths(output))) == 0.95){break}}
#  }

  median(lengths(output))
  median(lengths(output2))
  boxplot(lengths(output),lengths(output2), col = "light grey", outpch = 3)
  wilcox.test(lengths(output), lengths(output2))
  ...
  repeat{
    result2 <- cluster_label_prop(graph)
    output2 <- c(output2, list(result2))
    if(length(which(lengths(output2) == median(lengths(output2)))) == n/10){break}
  }
  ...
  for(i in 1:length(V(graph)$name)){
    ...
    V(graph)$community_model[V(graph)$name == 
                               names(membership(output[[l]])[i])] <- median(test)
  }
...
Gdata_WS <- Gdata_WS[-which(is.na(Gdata_WS[,4])),-c(53:ncol(Gdata_WS))] # Zeilen und Spalten loeschen
...
for (r in 1:nrow(Gdata_anonym)){
  ...
  if (Gdata_anonym[r,4] == "Wirtschaft"){Gdata_anonym[r,4] <- "Business"}
...
Gdata_anonym_inst <- Gdata_anonym[,-1]
Gdata_anonym_inst_subset <- Gdata_anonym[,-1]
Gdata_anonym_inst[,c(4:45)] <- 0
Gdata_anonym_inst <- Gdata_anonym_inst[-which(duplicated(Gdata_anonym_inst)),]
for(w in c(4:45)){
  list1 <- Gdata_anonym_inst_subset[which(Gdata_anonym_inst_subset[,w] != 0),c(1:3,w)]
  list2 <- as.data.frame(table(list1))
  list2 <- list2[which(list2$Freq != 0),-4]
  names(list2) <- c("inst", "country", "category", paste("WS", w-3, sep=""))
  for(r in c(1:nrow(list2))){
    Gdata_anonym_inst[which(Gdata_anonym_inst$inst == list2[r,1]),w] <- list2[r,4]
  }
}
rm(w,r,list1,list2,Gdata_anonym_inst_subset)
Gdata_anonym_inst <- Gdata_anonym_inst[order(rev(Gdata_anonym_inst$category), Gdata_anonym_inst$country, Gdata_anonym_inst$inst),]

xlsx::write.xlsx(Gdata_anonym_inst, file = "Dataset.xls") ### EDIT
...
Raw_countries <- xlsx::read.xlsx(file = "countries.xlsx", sheetIndex = 1, encoding="UTF-8")
Raw_countries$CC <- as.factor(countrycode::countrycode(Raw_countries$country, origin = "country.name", destination = "genc2c"))

countries <- Raw_countries[-c(533:nrow(Raw_countries)),-c(2,3)]
countries <- joinCountryData2Map(countries, joinCode = "ISO2", nameJoinColumn = "Var1")
...
legend("bottomright", 
       ...
       legend = c("No category", "Church", "Education", "Media", "NGOs", "Business", "Public authority", "Science"),
...
#-1.8,-0.2
legend(0,1, title = "Communities", legend = c("Biodiversity science-policy-interface Germany", 
                                                           "International expert workshop for IPBES (WS10)",
                                                           "1st Pan-European IPBES Stakeholder Consultation (PESC-1; WS18)", 
                                                           "2nd Pan-European IPBES Stakeholder Consultation (PESC-2; WS23)", 
                                                           "3rd Pan-European IPBES Stakeholder Consultation (PESC-3; WS27)", 
                                                           "4th Pan-European IPBES Stakeholder Consultation (PESC-4; WS31)",
                                                           "5th Pan-European IPBES Stakeholder Consultation (PESC-5; WS37)",
                                                           "Evening event on the concept of nature (WS36)",
                                                           "Evening event on the human-nature relationship (WS40)"), 
       fill = c("green", "blue", "#7947d5", "#739fcd", "#4d378f", "#867dd6", "#494f7d", "khaki", "peru"), # 7 und 8 vertauscht
       border = NA,
       bty = "n")
#-1.8,1.2
legend(0,0.4, title = "General network features", 
...
plot.igraph(ws2, vertex.size = V(ws2)$vertex.size, vertex.color = V(ws2)$color_k, vertex.shape = V(ws2)$vertex.shape,
     ...
     edge.arrow.size = 0, edge.color = E(ws2)$color_k,
     layout = lyout_ws, add = TRUE)
legend(-1.8,-0.2, title = "Institution details", legend = c("Science", "Public authority", "Business", 
                                                  "NGO", "Media", "Education", 
