...
clp_model <- function(graph, times = 100){
  ...
  output <- list()
  for(i in c(1:times)){
  result <- cluster_label_prop(ws2)
  output <- c(output, list(result))
  }
  median(lengths(output))
  boxplot(lengths(output), col = "light grey", outpch = 3)
  output <- output[-which(lengths(output) != median(lengths(output)))]
  
  for(i in 1:nrow(nodes_ws2)){
    for(l in 1:median(lengths(output))){
      test <- vector()
      add <- membership(output[[l]])[i]
      test <- c(test, add)
    }
    V(ws2)$community_model[V(ws2)$name == names(membership(output[[l]])[i])] <- median(test)
  }
  
  clp <- make_clusters(ws2, membership = V(ws2)$community_model)
...
## Farben und Größe
E(ws2)$color_st <- "grey"
E(ws2)$color_k[E(ws2)$no_participants > 6] <-  "black" #rgb(0,0,0,maxColorValue=255,alpha=1)
E(ws2)$color_k[E(ws2)$no_participants %in% c(2:5)] <- "dark grey" #rgb(169,169,169,maxColorValue=255,alpha=0.8)
E(ws2)$color_k[E(ws2)$no_participants == 1] <- "light grey" #add.alpha("light grey", alpha = 0.3)
...
#cairo_pdf("Rplot_international_test.pdf")
par(mfrow=c(1,1))
