...
boxplot(lengths(output))
output <- output[-which(lengths(output) != median(lengths(output)))]

repeat {
  clp <- cluster_label_prop(ws2)
  if (length(clp) == median(lengths(output))){
    break
    }
}
...
V(ws2)$wsnames <- V(ws2)$name
V(ws2)$wsnames[V(ws2)$type == "institution"] <- NA
V(ws2)$wsnames[V(ws2)$community == 1] <- NA
colrs <- brewer.pal(n = length(clp), name = "Paired")
V(ws2)$comm_col <- colrs[V(ws2)$community]
