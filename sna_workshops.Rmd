...
Gdata <- subset_workshops(Gdata_WS, subset = "all")
```
...
rm(rows1, rows2)
xlsx::write.xlsx(workshops_sna, file = "SNA_Statistik_Workshops.xls")
...
## Workshops alle
E(ws2)$color <- "grey"
V(ws2)$color[V(ws2)$type == "workshop"] <- "dark grey" # Workshops
V(ws2)$color[V(ws2)$type == "institution"] <- farben_ws1 # Institutionen
#V(ws2)$name[V(ws2)$type == "workshop"] <- NA # Workshops
...
par(mfrow=c(1,1))
plot(ws2, vertex.size = 3, vertex.color = V(ws2)$color,
     vertex.label = V(ws2)$name, vertex.label.family = "Calibri",
     vertex.frame.color = NA, vertex.label.color = "black",
     edge.arrow.size = 0,
     layout = lyout_ws)

# Workshops alle: Kategorien
V(ws2)$color[V(ws2)$type == "workshop"] <- "dark grey" # Workshops
V(ws2)$color[V(ws2)$kategorie == "Wissenschaft"] <- rgb(92,139,255,max=255)
V(ws2)$color[V(ws2)$kategorie == "Behoerde"] <- rgb(219,174,0,max=255)
V(ws2)$color[V(ws2)$kategorie == "Wirtschaft"] <- rgb(11,33,94,max=255)
V(ws2)$color[V(ws2)$kategorie == "Verbaende"] <- rgb(0,155,68,max=255)
V(ws2)$color[V(ws2)$kategorie == "Medien"] <- rgb(180,33,0,max=255)
V(ws2)$color[V(ws2)$kategorie == "keine Angabe"] <- rgb(255,161,213,max=255)
E(ws2)$color <- "grey"
