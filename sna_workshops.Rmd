...
length(unique(abbr[,3]))
length(unique(Gdata_WS[,2]))
#toadd <- as.vector(Gdata_WS[which(unique(Gdata_WS[,2]) %!in% unique(abbr[,3])),2])
...
workshops_sna[rows1,9] <- round(workshops_sna[,5] / max(workshops_sna[,5]),digits=2) # betweenness1
workshops_sna[,10] <- round(workshops_sna[rows1,6] / max(workshops_sna[rows1,6]),digits=2) # degree2
...
for (ws in c(1:37)){
  WS_av[ws,7] <- nrow(subset(links_ws2, links_ws2$from == paste("ws", ws, sep = "")))
}

sum(WS_av[which(WS_av$Jahr == "2018"),7])
...
xy <- barplot(prop.table(t(zeitlich_ws[,c(2:7)]), 2)*100,
        xaxt = "n",
...
abline(h = mean(zeitlich_ws$Wissenschaft_rel), lty = 3, xpd=F)
text(cex=1, x=max(xy)+3, y=mean(zeitlich_ws$Wissenschaft_rel), "Ø Wissenschaft", xpd=TRUE, pos = 4)
...
legend("topright", inset=c(-0.3,0),
       legend = c("Wissenschaft", "Behörde", "Wirtschaft", "Verbände", "Medien", "keine Angabe"),
...
V(ws2)$label.cex[V(ws2)$type == "workshop"] <- 1 # Workshops
...
plot(ws2, vertex.size = 3, vertex.color = V(ws2)$color_k,
     ...
     layout = lyout_ws)
legend(-1.6,-0.3, title = "Kategorie", legend = c("Wissenschaft", "Behörde", "Wirtschaft", 
