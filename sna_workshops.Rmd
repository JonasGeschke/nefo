...
Gdata <- subset_workshops(Gdata_WS, subset = "2018")
```
...
rm(rows1, rows2)
xlsx::write.xlsx(workshops_sna, file = "SNA_Statistik_Workshops_2018.xls") ### EDIT
...
```{r Betweenness über Degree - NOT WORKING, eval=FALSE}
regr <- data.frame(degree = workshops_sna[-c(1:4),4], betweenness = workshops_sna[-c(1:4),5])


fit <- glm(y ~ log(x))
summary(fit)
newdata <- data.frame(degree = seq(min(x), max(x), length = length(x)))
...
lines(prd, newdata$degree, col = "green")
