...
document <- data.frame(matrix(0, ncol = (length(unique(tidy_SDGs$word))+1), nrow = 3))
document[1,1] <- "NBS"
document[2,1] <- "NSO"
document[3,1] <- "DNS"
names(document) <- c(NA, unique(tidy_SDGs$word))
col <- c(2:ncol(document))
for (c in col){
  if (as.character(names(document[c])) %in% tidy_NBS$word){
    document[1,c] <- length(which(SDGs[c-1] == NBS))}
  if (as.character(names(document[c])) %in% tidy_NSO$word){
    document[2,c] <- length(which(SDGs[c-1] == NSO))}
  if (as.character(names(document[c])) %in% tidy_DNS$word){
    document[3,c] <- length(which(SDGs[c-1] == DNS))}
}


test <- tidy(document)

book_words <- list(tidy_NBS, tidy_NSO, tidy_DNS) %>%
